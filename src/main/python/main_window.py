...
class MainWindow(QtWidgets.QWidget):
    ...
    def convert_images(self):
        ...
        size = self.spn_quality.value() / 100.0
        folder = self.le_dossierOut.text()

        lw_items = [self.lw_files.item(index) for index in range(self.lw_files.count())]
        self.images_a_convertir = [1 for lw_item in lw_items if not lw_item.processed]
        if not self.images_a_convertir:
            msg_box = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, "Problème lors de la conversion.",
        ...
        self.thread.start()
        if len(self.images_a_convertir) != 1:
            self.prg_dialog = QtWidgets.QProgressDialog("Conversion des images", "Annuler", 1, len(self.images_a_convertir))
